...
public class BookingController : ControllerBase
{
    ...
    static TravelRequestResponseDto ToDto(TravelRequest r) => new()
    {
        ...
        LocationCapturedAt = r.LocationCapturedAt,
    ...
    {
        var q = _db.TravelRequests.Include(r => r.Employee)
                   .Include(r => r.ExpenseClaims).ThenInclude(e => e.Employee).AsQueryable();
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        var empId = CurrentEmployeeId;
        if (CurrentRole == "Employee" && empId > 0 && r.EmployeeId != empId) return Forbid();
        return Ok(ToDto(r));
    }

    // ── Create: saves location fields ─────────────────────────────────────────
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateTravelRequestDto dto)
    {
        var empId = dto.EmployeeId > 0 ? dto.EmployeeId : CurrentEmployeeId;
        ...
        await _notify.NotifyAdminsAndHrAsync(empId, "TravelRequest",
            ...
            (dto.OriginAddress != null ? $" from {dto.OriginAddress[..Math.Min(40, dto.OriginAddress.Length)]}" : ""),
