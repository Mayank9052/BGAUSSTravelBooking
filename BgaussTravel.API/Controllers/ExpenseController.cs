...
public class ExpenseController : ControllerBase
{
    ...
    // GET /api/Expense/summary  → DashboardPage stats
    [HttpGet("summary")]
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        if (CurrentRole=="Employee" && e.EmployeeId!=CurrentEmployeeId) return Forbid();
    ...
    public async Task<IActionResult> Create([FromBody] CreateExpenseClaimDto dto)
    {
        ...
        if (CurrentRole=="Employee" && request.EmployeeId!=CurrentEmployeeId) return Forbid();

        var code = await _seq.NextAsync("EXP");
        var claim = new ExpenseClaim
        {
            RequestId=dto.RequestId, EmployeeId=CurrentEmployeeId, ClaimCode=code,
        ...
        await _notify.NotifyAdminsAndHrAsync(CurrentEmployeeId, "ExpenseClaim",
            "New Expense Claim",
    ...
    public async Task<IActionResult> UploadBill(int id, IFormFile file)
    {
        ...
        if (CurrentRole=="Employee" && claim.EmployeeId!=CurrentEmployeeId) return Forbid();
    ...
    public async Task<IActionResult> Approve(int id, [FromBody] ApprovalActionDto dto)
    {
        ...
        claim.ApprovedBy=CurrentEmployeeId; claim.ApprovedAt=DateTime.UtcNow;
        claim.RejectionReason=action=="reject"?dto.Comments:null;
        claim.UpdatedAt=DateTime.UtcNow;
        await _db.SaveChangesAsync();

        await _notify.NotifyUserAsync(claim.EmployeeId, CurrentEmployeeId, "ExpenseClaim",
    ...
    public async Task<IActionResult> Reimburse(int id)
    {
        var claim = await _db.ExpenseClaims.FindAsync(id);
        if (claim==null) return NotFound();
        if (claim.Status!="Approved") return BadRequest(new{message="Only approved claims can be reimbursed."});
        claim.Status="Reimbursed"; claim.ReimbursedAt=DateTime.UtcNow; claim.UpdatedAt=DateTime.UtcNow;
        await _db.SaveChangesAsync();
        await _notify.NotifyUserAsync(claim.EmployeeId, CurrentEmployeeId, "Reimbursement",
